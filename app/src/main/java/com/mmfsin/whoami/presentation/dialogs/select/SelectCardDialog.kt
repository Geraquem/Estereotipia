...
@AndroidEntryPoint
class SelectCardDialog(private val cardId: String, private val gameReady: Boolean) :
    BaseDialog<DialogCardSelectBinding>() {

    private val viewModel: SelectCardDialogViewModel by viewModels()
    ...
    private fun observe() {
        viewModel.event.observe(this) { event ->
            when (event) {
                is SelectCardDialogEvent.GetPeopleCardDialog -> {
                    this.card = event.card
                    setUI()
                }
                is SelectCardDialogEvent.SomethingWentWrong -> error()
            }
        }
    }
    ...
    companion object {
        fun newInstance(cardId: String, gameReady: Boolean = false): SelectCardDialog {
            return SelectCardDialog(cardId, gameReady)
        }
    }
}